...
def building_1055():
    ...
    # Display AC units in Building 1055
    if 'Building' in df.columns:
        building_1055_units = df[df['Building'] == 1055]
        st.write("## AC Units in Building 1055")
        if not building_1055_units.empty:
            st.dataframe(building_1055_units)

            # Display details for a selected unit
            unit_ids = building_1055_units['RTU'].tolist()
            selected_unit = st.selectbox("Select an AC Unit", unit_ids, key="unit_select")
            if selected_unit:
                unit_details = building_1055_units[building_1055_units['RTU'] == selected_unit].iloc[0]
